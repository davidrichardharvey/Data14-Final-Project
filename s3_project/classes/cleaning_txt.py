...
class TextFiles:
    ...
    def get_scores(self):
        # Splits the presentation and psychometric scores into score and max scores
        # Also formats the name to title casing
        for item in self.results:
        ...
        for name in self.results:
            if " " in list(name['first_name']):
                with open(find_variable("issues", "ISSUE FILES"), "a") as text_file:
                    text_file.writelines(f"FileName: Sparta Day {' '.join(name['date'].split()[1:])}.txt"
